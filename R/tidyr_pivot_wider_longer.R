...
sg_data <- bind_rows(
  ...
  mutate(frame = factor(frame, labels = c('pivot_wider(long, id, key, val)', 'pivot_longer(wide, x:z, key, val)'))) %>%
...
anim_save(here::here("images", "tidyr-pivot_wider_longer.gif"), sg_anim)
