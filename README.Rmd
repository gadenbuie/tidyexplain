...
ggsave(g, file = here::here("images/original-dfs.png"), dpi = "screen")
```
...
g <- plot_data(initial_dfs %>% filter(label != "id")) +
  geom_text(data = df_names, family = "Fira Mono", size = 24)

ggsave(g, file = here::here("images/original-dfs-so.png"), dpi = "screen",width = 7, height = 5)
x <- x %>% select(-id)
y <- y %>% select(-id)
```
